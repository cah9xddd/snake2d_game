...
int GameManager::CheckSquare(Vector2<int> square) const { return field[square.x][square.y]; }
...
void GameManager::SetDifficulty(DIFFICULTY_ new_difficulty) { difficulty = new_difficulty; }

DIFFICULTY_ GameManager::GetDifficulty() const { return difficulty; }

void GameManager::SetGameState(GAME_STATE_ new_game_state) { game_state = new_game_state; }

GAME_STATE_ GameManager::GetGameState() const { return game_state; }

int GameManager::GetScore() const { return score; }

void GameManager::AddToScore(int num) { score += num; }
